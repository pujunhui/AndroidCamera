 ...
 * surfaceSize，display旋转后的尺寸，即画布大小，最终可绘制的区域是surface和display交集
 *
 * @param cameraRotate 表示camera画面旋转多少度后，才能正确在当前屏幕上显示
 * @param cameraSize camera预览尺寸（camera数据流大小）,一般不会改变
 * @param displaySize 显示控件（TextureView）大小，跟随Surface改变
